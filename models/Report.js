...
static async getRecentOrders(limit = 5) {
  ...
  static async getFinancialSummary(startDate, endDate) {
    try {
      ...
      const result = await db.queryOne(`
        SELECT 
          COALESCE(SUM(CASE WHEN payment_status = 'paid' AND payment_time BETWEEN ? AND ? THEN total_amount ELSE 0 END), 0) as total_revenue,
          COALESCE(SUM(CASE WHEN payment_status = 'paid' AND payment_time BETWEEN ? AND ? THEN tax ELSE 0 END), 0) as vat_collected,
          COALESCE(SUM(CASE WHEN payment_status = 'paid' AND payment_time BETWEEN ? AND ? THEN tip ELSE 0 END), 0) as tips_collected,
          COALESCE(SUM(CASE WHEN payment_status = 'paid' AND payment_time BETWEEN ? AND ? THEN discount ELSE 0 END), 0) as discounts_given,
          COUNT(CASE WHEN payment_status = 'paid' AND payment_time BETWEEN ? AND ? THEN 1 END) as transaction_count,
          AVG(CASE WHEN payment_status = 'paid' AND payment_time BETWEEN ? AND ? THEN total_amount END) as avg_transaction_value,
          COALESCE(SUM(CASE WHEN payment_status = 'paid' AND payment_time BETWEEN ? AND ? THEN (total_amount + COALESCE(tax, 0) + COALESCE(tip, 0) - COALESCE(discount, 0)) ELSE 0 END), 0) as total_collected,
  ...
  static async getProfitLossStatement(startDate, endDate) {
    try {
      ...
      // FIX: was this.getEmptyP&LStatement() — the & made JS parse this as a
      // bitwise AND expression, throwing ReferenceError and crashing the process
      return this.getEmptyPLStatement();
    }
  }
  ...
  static async getVATReport(startDate, endDate) {
    try {
      ...
      // FIX: getEmptyVATReport was called but never defined — threw TypeError and crashed process
      ...
      return {
        ...
        efficiency: {
          ...
          food_cost_percentage: revenueData.total_revenue > 0 ?
            (expenseData.total_cogs / revenueData.total_revenue) * 100 : 0,
          // FIX: getTableTurnoverRate/getRevenueGrowth/getTransactionGrowth/getCustomerGrowth
          // were all called but never defined — stubbed with 0 to prevent crash
  ...
  // FIX: was named getEmptyP&LStatement — & is not valid in a method name
  static getEmptyPLStatement() {
    return {
      period: {},
      revenue: { total_revenue: 0, vat_collected: 0, net_revenue: 0, tips_collected: 0, discounts_given: 0, transaction_count: 0, avg_transaction_value: 0 },
      cost_of_goods_sold: { total_cogs: 0, food_cost: 0, beverage_cost: 0, packaging_cost: 0, cogs_percentage: 0 },
      gross_profit: { amount: 0, margin: 0 },
      operating_expenses: { total: 0, labor: 0, rent: 0, utilities: 0, marketing: 0, maintenance: 0, other_operating: 0, percentage_of_revenue: 0 },
      operating_profit: { amount: 0, margin: 0 },
      other_income_expenses: { total: 0, interest: 0, depreciation: 0, other: 0 },
      net_profit: { amount: 0, margin: 0, is_profitable: false }
    };
  }

  // FIX: this method was called in getVATReport's catch block but was never defined
  static getEmptyVATReport() {
    return {
      period: {},
      summary: {
  ...
  static getEmptyExpenseData() {
    return {
      food_cost: 0, beverage_cost: 0, packaging_cost: 0, total_cogs: 0,
      labor_cost: 0, rent: 0, utilities: 0, marketing: 0, maintenance: 0,
      other_operating: 0, total_operating_expenses: 0,
      interest: 0, depreciation: 0, other_expenses: 0, total_other_expenses: 0
    };
  }
